    ...
    <nav aria-label="breadcrumb" class="mob-mt-15">
      <small> 
        <ol class="breadcrumb bg-white text-warning p-0">
          ...
          <li class="breadcrumb-item"><a href="{{Request::url()}}">{{ $section }}</a></li>
        ...
        <div class="row m-0">
          ...
          <div class="col-lg-8 col-md-12 mob-p-0 mob-pt-10">
            ...
            <p class="date font-weight-bold text-muted mb-0 mt-0"> <i class="fas fa-user mr-1"></i> 
              <a href="{{url('author').'/'.str_slug($sA->authorname).'-'.$sA->author_id}}">@if($sA->authorname != 'Samachar4media Bureau' || !isset($sA->authorname)){{$sA->authorname}}@else समाचार4मीडिया ब्यूरो ।।@endif </a> <i class="far fa-clock ml-2 mr-1"></i> {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $sA->pickdate)->diffForHumans()}} 
            </p>
            <p class="mt-3">
