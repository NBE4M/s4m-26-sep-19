...
<div class="row mob-p-0 mob-m-0">
  ...
  <div class="col-md-7 col-lg-9 dashed-bdr-r mob-bdr-0 mob-p-0 pl-md-0 pl-lg-3">
    ...
    @if(isset($ArrlistingArticles))  
    <div class="row m-0">
      @foreach($ArrlistingArticles as $k=>$ala)
      <div class="row m-0">
        ...
        <div class="row m-0">
          <div class="col-lg-4 col-md-12 p-0">
            <a href="{{$ala->url}}">
              <img class="img-fluid img-thumbnail" src="{{Config::get('constants.storagepath')}}{{$ala->photopath}}" alt="{{$ala->phototitle}}" title="{{$ala->phototitle}}" onerror="this.onerror=null;this.src='images/new-2.jpg';">
            </a>
          </div>
          <div class="col-lg-8 col-md-12 mob-p-0 mob-pt-10">
            ...
            <p class="date font-weight-bold text-muted mb-0 mt-0"> <i class="fas fa-user mr-1"></i> 
              <a href="{{url('author').'/'.$ala->authorname.'-'.$ala->author_id}}"> @if($ala->authorname != 'Samachar4media Bureau' || !isset($ala->authorname)){{$ala->authorname}}@else समाचार4मीडिया ब्यूरो ।।@endif</a> <i class="far fa-clock ml-2 mr-1"></i> <a href="#">{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ala->pickdate)->diffForHumans()}} </a>
            </p>
            <p class="mt-3">
