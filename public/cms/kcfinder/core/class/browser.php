...
class browser extends uploader {
    ...
    public function action() {
        ...
        if (!isset($this->session['dir']))
            ...
            $dir = 'https://storage.googleapis.com/news-photo/cms/kcfinder/upload' . "/" . $this->session['dir'];
    ...
    protected function act_upload() {
        ...
        if (is_array($this->file['name'])) {
            ...
            $url= 'https://www.samachar4media.com/api/insertimg?img='.$name;
            $data1 = array($url);
            $this->multiRequest($data1);
            }
            return implode("\n", $return);
        } else
