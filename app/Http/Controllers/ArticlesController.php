...
class ArticlesController extends Controller {
    ...
    public function index($option) {
        ...
        switch ($option) {
            ...
            case "priority":
                $status = 'P';
                $rightLabel = "priorityArticles";
                $rightId = 17;
                break;
            case "custom":
                $status = 'P';
                $rightLabel = "customArticles";
                $rightId = 98;
                break;
        }
        /* Right mgmt start */
        ...
        $data1  = DB::table('custom_section')
                    ->select(DB::raw('custom_section.*'))
                    ->orderby('sequence','asc')->orderby('publish_date','desc')->limit(100)->get();           
          if (count($data) > 0) {
              return view('articles.' . $option, compact('articles','data','articlestop', 'editor', 'channels', 'currentChannelId','newstype','selectedNewType','data1','margin'));
          }else{
             $data  = DB::table('article_feature')
                    ->select(DB::raw('article_feature.*'))
                    ->orderby('sequence','asc')->orderby('publish_date','desc')->limit(100)->get();
...
//        
    ...
    public function update(Request $request) {
       // dd($request);
        ...
        if ($request->status == 'S') {
            $check1 = DB::table('articles')->where([['title',$request->title],['article_id','!=',$request->id]])->count();
           
            if ($check1 > 0) {
                Session::flash('message', 'Title Already Exist.');
                return redirect()->back()->with('message', 'Title Already Exist.');
            }
        }
        ...
        if ($request->status == 'N') {
           $check = DB::table('articles')->where([['slug',$request->url],['article_id','!=',$request->id]])->count();
...
/*after first time publish*/
        ...
        if ($article->status =='P') {
                ...
                $EventLogController->eventstore(['userid'=>$uid,'event_type'=>'article_feature_create','description'=>'Create Feature Article','article_id'=>$request->id]);
                Cache::forget('homeSlide');
    ...
    public function store(Request $request) {
        ...
        if ($request->status == 'S') {
            ...
            if (!empty($request->pdfFile)) {
                $validator2 = Validator::make($request->all(), [
            
                'pdfFile' => 'required|mimes:pdf|max:5242880',            
                ]);
                if ($validator2->fails()) {
                    return redirect('article/create')
        ...
        try {
            ...
            $article->is_columnist = $article->author_type == '4' ? 1 : 0;
            $article->title = $request->title;
            $article->pr_name = $request->pname ? $request->pname:'';
            $article->pr_email = $request->pemail ? $request->pemail: '';
            $article->pr_number = $request->pmobile ? $request->pmobile:'';
            $title = $request->title;            
            $article->summary = $request->summary;
            $description = $request->description.'<p><strong>आप अपनी राय, सुझाव और खबरें हमें mail2s4m@gmail.com पर भेज सकते हैं या 01204007700 पर संपर्क कर सकते हैं।&nbsp;(हमें&nbsp;<a href="https://www.facebook.com/Samachar4media/" target="_blank" style="color:blue">फेसबुक</a>,<a href="http://xn--%20-fvda2exa4d2e/" target="_blank">&nbsp;</a><a href="https://twitter.com/samachar4media" target="_blank" style="color:blue">ट्विटर</a>,<a href="https://www.linkedin.com/company/samachar4media-com/" target="_blank" style="color:blue">&nbsp;लिंक्डइन</a>&nbsp;और<a href="https://www.youtube.com/channel/UCqMEhxJQQaFwLH4PUtXjQcw" target="_blank" style="color:blue">&nbsp;यूट्यूब</a>&nbsp;पर फॉलो करें)</strong></p>';
            if ($request->status == 'P' || $request->status == 'SD') {
            ...
            if ($request->status == 'SD') {
                ...
                if ($request->important=='f') {
                    ...
                    $articlef->save();
                    $EventLogController = new EventLogController;
                    $EventLogController->eventstore(['userid'=>$uid,'event_type'=>'article_feature_create','description'=>'Create Feature Article','article_id'=>$artid]);
                    Cache::forget('homeSlide');
            }
                $article->publish_date = date('Y-m-d');
                $article->publish_time = date('H:i:s');
    ...
    public function destroy() {
        ...
        foreach ($delArr as $d) {
            ...
            if ($deleteAl->status != 'S') {
                ...
                $deleteAl->save();
                ArticleF::where('story_key_id',$d)->delete();
            } else {
    ...
    public function destroyfeaturenews() {
        ...
        $EventLogController = new EventLogController;
        $EventLogController->eventstore(['userid'=>$uid,'event_type'=>'article_feature_delete','description'=>'Feature Article Remove','article_id'=>$d]);
        }
    ...
    public function destroycustomnews() {
        if (!Session::has('users')) {
            return 'Please login first.';
        }
        $id = Session::get('users');
        ...
        $delArr = explode(',', $id);
        foreach ($delArr as $d) {
        DB::table('custom_section')->where('story_key_id',$d)->delete();
        DB::table('articles')->where('article_id',$d)->update(['is_pinned'=>0]);
        $EventLogController = new EventLogController;
        $EventLogController->eventstore(['userid'=>$uid,'event_type'=>'custom_remove','description'=>'Custom Article Remove','article_id'=>$d]);
        }
    ...
    public function deleteUnusedImages()
{
 ...
 public function sortarticle(Request $request) {
    ...
    public function sortCarticle(Request $request) {
        foreach($request->item as $k => $itm){
            $newsLetterArticle=CustomSection::find($itm);
            $newsLetterArticle->sequence=$k+1;
            $newsLetterArticle->updated_at = date('Y-m-d H:i:s');
            $newsLetterArticle->save();
            $this->forget($itm);
...
public function sendmail($title,$id,$url,$summary,$photopath){
        ...
        foreach($mails as $key => $value){
          ...
          $cc = explode(',',$exp[1][1]);
// dd($cc);
        Mail::send('emails.publish_email', ['name' =>Session::get('users')->name,'url'=>$url,'title'=>$title,'summary'=>$summary,'photopath'=>$photopath], function ($message) use ($title,$to) {
...
// update  Description Ads Article
// pdf/video files upload function
public function pdfFileUpload(Request $request)
{
    if (isset($_POST['id'])) {
        $articlelastid = DB::table('articles')->select('article_id')->where('article_id',$_POST['id'])->get();
        $artid = $articlelastid[0]->article_id;
        }else{
        $articlelastid = DB::table('articles')->select('article_id')->orderby('article_id','desc')->limit(1)->get();
        $artid = $articlelastid[0]->article_id+1;
        }
    $pdfName = $artid.'-'.$request->file('pdffiles')->getClientOriginalName();
    $request->file('pdffiles')->move(public_path('files/pdf'), $pdfName);
    Storage::disk('gcs')->put($pdfName,file_get_contents(config('constants.PDFfilespath').$pdfName),
                    \Illuminate\Contracts\Filesystem\Filesystem::VISIBILITY_PUBLIC
                    );
...
public function pdfFileDelete($name)
{
    File::delete(public_path("files/pdf/". $name));
...
public function assignarticle()
{
    ...
    if (isset($_POST['option'])) {
        ...
        $createArr = explode(',', $id);
      
    foreach ($createArr as $d) {
        $check = CustomSection::where('story_key_id',$d)->first();
        if (isset($check)>0) {
            return response()->json(['error'=>'Article already present on custom section.']);
        }
    $articleDetails = DB::table('article')->where('article_id',$d)->first();
    DB::delete('delete from custom_section where sequence >= 10');
        $articleC = new CustomSection();
        $articleC->story_key_id = $articleDetails->article_id;
        $articleC->title = $articleDetails->title;
        $articleC->url = $articleDetails->url;
        $articleC->photopath = Config::get('constants.storagepath').$articleDetails->photopath;
        $articleC->phototitle = isset($articleDetails->phototitle) ? Config::get('constants.storagepath').$articleDetails->phototitle : '';
        $articleC->category_hname = $articleDetails->category_hname;
        $articleC->category_ename = $articleDetails->category_name;
        $articleC->sequence = '0';
        $articleC->story_type = 'article';
        $articleC->publish_date = date('Y-m-d H:i:s');
        $articleC->created_at = date('Y-m-d H:i:s');
        $articleC->updated_at = date('Y-m-d H:i:s');
        $articleC->save();
        DB::table('articles')->where('article_id',$d)->update(['is_pinned'=>1]);
        $EventLogController = new EventLogController;
        $EventLogController->eventstore(['userid'=>$uid,'event_type'=>'custom_create','description'=>'Create Custom Article','article_id'=>$articleDetails->article_id]);
        // Cache::forget('customSection');
                }
}
/*cache*/
