    ...
    */
    ...
    public function __construct()
    {
        ...
        if (Cache::has('breaking')) {
            ...
            Cache::put('breaking', $breaking, 1440);
        };        
         if (Cache::has('polls')) {
        ...
        if (Cache::has('industry-briefing')) {
            ...
            $industryBriefing = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','url','photopath','phototitle','category_name','category_hname')->where([['category_id',3],['web_exclusive',0]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(4)->get();
            Cache::put('industry-briefing', $industryBriefing, 1440);
        };
        if (Cache::has('social-media')) {
            ...
            Cache::put('social-media', $socialM, 1440);
        };
        ...
        if (Cache::has('most-read')) {
            ...
            $mostRead = Mostread::Join('article','article_most_read.article_id','=','article.article_id')->select(DB::raw('CONCAT(article.publish_date,article.publish_time) AS pickdate,article.article_id,article.title,article.url,article.publish_date,article.photopath,article.phototitle,article.category_hname,article.category_name,article.summary'))
            ->where('article_most_read.article_id','!=',0)
            ->where('article.publish_date','>=',$lastsevenday)
            ->orderBy('article_most_read.number_hits','desc')
            ->take(6)->distinct()->get();
            Cache::put('most-read', $mostRead, 1440);
        }; 
        if (Cache::has('vicharmanch')) {
            ...
            Cache::put('vicharmanch', $vicharmanch, 1440);
        };
        if (Cache::has('videos')) {
            $videos= Cache::get('videos');
        }else{
            $videos = Ytube::orderby('publish_date','desc')->take(5)->get();
            Cache::put('videos', $videos, 1440);
        };
        if (Cache::has('frontalbum')) {
            ...
            Cache::put('frontalbum', $frontalbum, 1440);
        };
    ...
    public function index(Request $request)
    {
        ...
        if (Cache::has('homeSlide')) {
        	...
        	Cache::put('homeSlide', $homeSlide, 1440);
        };
        
        if (Cache::has('customSection')) {
            $customSection= Cache::get('customSection');
        }else{
            $customSection = DB::table('custom_section')->select('title','url','photopath','phototitle','category_ename','category_hname','sequence')->where('sequence','!=', 0)->orderby('sequence','asc')->limit(4)->get();
            Cache::put('customSection', $customSection, 1440);
        };

        if (Cache::has('media-forum')) {
            $mediaForum= Cache::get('media-forum');
        }else{
            $mediaForum = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','url','category_name')->where([['category_id',4],['web_exclusive',0]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(6)->get();
            Cache::put('media-forum', $mediaForum, 1440);
        };
        ...
        if (Cache::has('admission-jobs')) {
            ...
            Cache::put('admission-jobs', $jobs, 1440);
        };        
        if (Cache::has('interviews')) {
            ...
            Cache::put('interviews', $interview, 1440);
        };
        ...
        if (Cache::has('brand-speaks')) {
            ...
            Cache::put('brand-speaks', $brandSpeaks, 1440);
        };
        if (Cache::has('advertisement')) {
            ...
            Cache::put('advertisement', $Adv, 1440);
        };
        ...
        if (Cache::has('poetry')) {
            ...
            Cache::put('poetry', $poetry, 1440);
        };
        if (Cache::has('print')) {
            $print= Cache::get('print');
        }else{
            $print = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','summary','url','photopath','phototitle','category_name','category_hname')->where([['category_id',75]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(4)->get();
            Cache::put('print', $print, 1440);
        };
        if (Cache::has('tv')) {
            $tv= Cache::get('tv');
        }else{
            $tv = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','summary','url','photopath','phototitle','category_name','category_hname')->where([['category_id',76]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(4)->get();
            Cache::put('tv', $tv, 1440);
        };
        if (Cache::has('digital')) {
            $digital= Cache::get('digital');
        }else{
            $digital = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','summary','url','photopath','phototitle','category_name','category_hname')->where([['category_id',77]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(4)->get();
            Cache::put('digital', $digital, 1440);
        };
        if (Cache::has('radio')) {
            $radio= Cache::get('radio');
        }else{
            $radio = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','summary','url','photopath','phototitle','category_name','category_hname')->where([['category_id',78]])->orderby('publish_date','desc')->orderby('publish_time','desc')->limit(4)->get();
            Cache::put('radio', $radio, 1440);
        };
    ...
    return view('welcome',compact('menus','homeSlide','industryBriefing','mediaForum','jobs','mostRead','vicharmanch','interview','brandSpeaks','Adv','mainNews','telS','socialM','videos','frontalbum','polls','poll_answer','breaking','parents','poetry','print','radio','tv','digital','customSection'));
    }

    public function section(Request $request,$section)
    {
        
        $sectionArticles = Article::select(DB::raw('CONCAT(publish_date,publish_time) AS pickdate'),'title','summary','url','authorname','author_id','photopath','phototitle','category_name')->where('category_name',$section)->orderby('publish_date','desc')->orderby('publish_time','desc')->paginate(15);
        ...
        if (Cache::has('sectionmostRead')) {
            ...
            Cache::put('sectionmostRead', $sectionmostRead, 1440);
        };
        $useragent = isset($_SERVER['HTTP_USER_AGENT'])
        ? strtolower($_SERVER['HTTP_USER_AGENT'])
        : '';
        ...
        if ($no != 0) {
            $title =  $meta->mtitle ? $meta->mtitle.' page no.'.$no:'';
            $metatitle = $meta->mtitle ? $meta->mtitle.' page no.'.$no:'';
            $metadescription = $meta->mdesc ? $meta->mdesc.' page no.'.$no:'';
        }else{
        ...
        $section = $meta->name;
        $canonical = $ogurl;
        return view('article.section', compact('menus','sectionArticles','sectionmostRead','industryBriefing','parents','metatitle','metadescription','metatag','ogimage','ogurl','section','title','canonical'));
    }
    ...
    {     
        $section = str_replace('-news',' ', $section);
                ...
                Cache::put('articles-'.$id, $articles, 1440);
                };
                ...
                Cache::put('articlesrelate-'.$id, $articlesrelate, 1440);
                };
                if (Cache::has('storymostRead')) {
                    ...
                    Cache::put('storymostRead', $storymostRead, 1440);
                };
          
            $article= $articles[0];
            $articleurl = preg_replace( "/\r|\n/", "", $article->url);
            $articlepath = explode('https://www.samachar4media.com/', $articleurl);

            if ($request->path() !== $articlepath[1]) {
      ...
      if(preg_match('/(ipad)/i',$useragent)||preg_match('/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i',$useragent)||preg_match('/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i',substr($useragent,0,4))){
        ...
        return view('article.story', compact('articles','articlesrelate','socialM','storymostRead','metatitle','title','metadescription','metatag','ogimage','ogurl','parents','canonical'));
    }
    ...
    public function photogallery(Request $request)
    {
        ...
        $shareAlbum = Album::Join('photos as p', 'album.id', '=', 'p.owner_id')
                          ->select(DB::raw('album.id,album.title as album_title,album.description as album_desc,p.title as photo_title,p.description as photo_desc,p.photopath,p.photo_id,album.updated_at as albumDate'))->where([['album.valid',1],['p.owned_by','album'],['p.active',1]])->groupby('album.id')->orderBy('p.photo_id', 'desc')->paginate(20);
        if ($no != 0){
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Photo, Image Gallery - Samachar4media'.' page no.'.$no;
            $title = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Photo, Image Gallery - Samachar4media'.' page no.'.$no; 
            $metadescription = 'Samachar4media meida, advertising, photo, image, picture gallery page India.'.' page no.'.$no;              
        } else {
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Photo, Image Gallery - Samachar4media';
            $title = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Photo, Image Gallery - Samachar4media' ; 
        ...
        return view('photogallery.album', compact('shareAlbum','ArrViewPhotos','metatitle','ogtitel','ogimage','ogurl','canonical','metadescription','ArrRecentFeatureNewsList','menus','arrmostread','ArrRecenttopnews','ArrRecentImportaintNewsHeaderList','ArrRecentNewsMiddelbarList','ArrMenuSLatestVideo','parents','title'));
    ...
    public function videoGallery(Request $request)
    {
        ...
        if ($no != 0) {
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media'.' page no.'.$no;
            $ogtitel =  ' मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media'.' page no.'.$no;
            $metadescription = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Samachar4media provides latest and breaking news videos and most searches and trending video in India.'.' page no.'.$no;
            $title = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media'.' page no.'.$no;
        } else {
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media';
            $ogtitel =  'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media';
            $metadescription = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Samachar4media provides latest and breaking news videos and most searches and trending video in India.';
            $title = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया | Video News, Most Searches and Trending Video, Video Gallery - Samachar4media';
        }
        ...
        return view('video.videoGallery', compact('galleryVideo','ArrRecentFeatureNewsList','menus','arrmostread','ArrRecentImportaintNewsHeaderList','ArrRecentNewsMiddelbarList','ArrMenuSLatestVideo','metatitle','title','ogtitel','ogimage','metatag','parents','metadescription'));
    }
    ...
    public function pollResult($id,Request $request)
    {
        ...
        $pollsre = DB::table('poll as p')->join('poll_answer as pa','p.id','=','pa.poll_id')->where([['p.id', $id],['p.status', '1']])->get();
        if ($pollsre->isEmpty()) {
           return Redirect::to('/',301)->with('This poll is not active !');
        }
        $pollarch = array();
        $pollsArch = DB::table('poll')->where('status',2)->orderBy('id', 'desc')->get();
        $paginatedItems = DB::table('poll')->where('status',2)->orderBy('id', 'desc')->paginate(10);
        ...
        return view('polls/pollresult',compact('menus','polls','poll_answer','sum','paginatedItems','parents','pollsre','title','metatitle','metadescription'));
    }
...
public function gdpr_page($value='')
    ...
    public function Tag_wise_Article_listing_page(Request $request,$name)
    {
        ...
        $ArrAuthorDetails='';
        if ($no != 0) {
           $metatitle = 'Editor Picks Of Advertising, TV Media Digital Marketing Agency - Samachar4media'.' page no'.$no;
           $title = 'Editor Picks Of Advertising, TV Media Digital Marketing Agency - Samachar4media'.' page no'.$no;
           $metadescription = 'Today top advertising news, media, marketing, digital news updates and trending topics story at Samachar4media.com'.' page no'.$no;
        } else {
           $metatitle = 'Editor Picks Of Advertising, TV Media Digital Marketing Agency - Samachar4media';
           $title = 'Editor Picks Of Advertising, TV Media Digital Marketing Agency - Samachar4media';
    ...
    public function Authorrepo_listing_page(Request $request)
    {
        ...
        $ArrAuthorDetails='';
        if ($no != 0) {
            $metatitle = 'Samachar4media All Author Profile Page'.' page no.'.$no;
            $title = 'Samachar4media All Author Profile Page'.' page no.'.$no;
            $metadescription = 'Samachar4media Author cover advertising, media, digital marketing, PR Industry and agency news in India'.' page no.'.$no;
        } else {
            $metatitle = 'Samachar4media All Author Profile Page';
            $title = 'Samachar4media All Author Profile Page';
     ...
     public function Author_wise_Article_listing_page(Request $request,$name,$id)
    {
       ...
       $authorbio =  DB::table('authors')->where('author_id', '=', $id)->where('valid', '=', 1)->first();
        ...
        $no = end($page);
        if ($no != 0) {
            $metatitle = $name.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
            $title =  $name.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
           $metadescription = $name.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
        } else {
            $metatitle = $name.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
        ...
        $ogdescription = $name.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
       | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. ';
    ...
    /*search*/
      ...
      "MATCH(title,tags) AGAINST(? IN NATURAL LANGUAGE MODE)",array($result))->where('publish_date','!=',null)->orderby('publish_date','desc')->orderby('publish_time','desc')->paginate(20);
        ...
        $no = end($page);
        if ($no != 0) {
            $metatitle = $result.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
            $title =  $result.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
           $metadescription = $result.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
        } else {
            $metatitle = $result.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. ';
        ...
        $ogdescription = $result.' - मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
       | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website.';
        $useragent = isset($_SERVER['HTTP_USER_AGENT'])
        ? strtolower($_SERVER['HTTP_USER_AGENT'])
        : '';
        ...
        public function tagarticle(Request $request,$result)
        {
...
// Session::flash('alert-class', 'alert-success'); 
    ...
    {     
        ...
        $no = end($page);
        if ($no != 0) {
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
            $title =  'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
           $metadescription ='मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. '.' page no.'.$no;
        } else {
            $metatitle = 'मीडिया, विज्ञापन और सोशल मीडिया की दुनिया की खबरों के लिए पढ़ें समाचार4मीडिया
           | Samachar4media has best and latest media news, advertising news, media industry updates for more information visit our website. ';
